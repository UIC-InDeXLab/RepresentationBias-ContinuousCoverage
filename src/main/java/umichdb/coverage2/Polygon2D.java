...
public class Polygon2D implements Shape, Cloneable, Serializable {
	...
	private void updatePath(double x, double y) {
		...
		if (path == null) {
			...
			if (y < _ymin)
				...
				_ymax = y;
			bounds = new Rectangle2D.Double(_xmin, _ymin, _xmax - _xmin,
					_ymax - _ymin);
		}
	}
	...
	/**
	 * Check if this polygon already has this point
	 * 
	 * @param x
	 * @param y
	 * @return
	 */
	private boolean containsExactPoint(double x, double y) {
		for (int i = 0; i < npoints; i++) {
